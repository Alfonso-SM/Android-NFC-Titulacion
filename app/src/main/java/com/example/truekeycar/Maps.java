...
public class Maps extends Fragment implements OnMapReadyCallback {
    ...
    private void getCurrentLoc() {
        mLocationClient.getLastLocation().addOnSuccessListener(location -> {
            ...
            if (!carIslocated)
                mGoogleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(MylatLng,14));
            direction();
        });
    ...
    private void goCarLocation() {
        ...
        longitude = 0;
        // DataBase
        Query checkUser = FirebaseDatabase.getInstance().getReference("NFC").child(LastCar);
